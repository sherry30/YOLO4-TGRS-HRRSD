...
def get_prediction_class_records(model, model_format, annotation_records, anchors, class_names, model_image_size, conf_threshold, elim_grid_sense, v5_decode, save_result):
    ...
    for (image_name, gt_records) in annotation_records.items():
        ...
        if save_result:
            ...
            testPath = os.path.join(result_dir, image_name.split("/")[-1])
            image.save(testPath)
...
def _generate_model(model_path, anchors, class_names, model_image_size):
        ...
        yolo_model, _ = get_yolo5_model("scaled_yolo4_csp_darknet", num_feature_layers, num_anchors, num_classes, input_shape=model_image_size + (3,), model_pruning=False)
        ...
        model_format='H5'
        return yolo_model, model_format
...
def main():
    ...
    model, model_format = _generate_model(args.model_path, anchors, class_names, model_image_size)
